...
requests["Find the year when the movie American Beauty released"] = "
  ...
  SELECT 
    actors.act_fname, actors.act_lname, movies.mov_title, 
  CAST
      (strftime('%Y', movies.mov_dt_rel) AS INTEGER) as mov_year 
  FROM 
    actors 
  INNER JOIN 
    movies_actors ON actors.id = movies_actors.act_id 
  INNER JOIN 
    movies ON movies_actors.mov_id = movies.id 
  WHERE CAST
    (strftime('%Y', movies.mov_dt_rel) AS INTEGER) NOT BETWEEN 1990 AND 2000 AND actors.act_fname <> 'Kevin' AND actors.act_lname <> 'Spacey';
"
